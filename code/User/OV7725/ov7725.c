...
static void FIFO_GPIO_Config(void)
{
    ...
    GPIO_Init(GPIOB, &GPIO_InitStructure);
		...
		/*PB0(FIFO_RRST--FIFO读复位) PB1(FIFO_OE--FIFO输出使能)*/
    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0 | GPIO_Pin_1;
    GPIO_Init(GPIOB, &GPIO_InitStructure); 

		/*PA1(FIFO_RCLK-FIFO读时钟)*/
...
ErrorStatus Ov7725_TryInit(void)
{
	...
	while(retry--)	
	{
		...
		{
			DEBUG("Init ov7725 success.\r\n");	
			return SUCCESS;
		}
		else
			DEBUG("Init ov7725 error.\r\n");
	}
	return ERROR;	//超时返回错误
}
